...
using TensorStructures = ::absl::Span<const std::vector<::tensorflow::Tensor>>;

std::string InputTensorName(size_t i, size_t element_index) {
  return absl::StrCat("structure_", i, "_element_", element_index);
}

const absl::string_view kOutputTensorName = "serialized_dataset";

template <typename T>
std::string MismatchedElementsMsg(absl::string_view property,
                                  size_t element_index, T first_value,
                                  T second_value, size_t second_position) {
  return absl::StrCat(
...
absl::StatusOr<std::pair<tf::DataType, tf::TensorShape>>
GetDtypeAndShapeForStructureElement(TensorStructures tensor_structures,
  ...
  for (size_t i = 0; i < tensor_structures.size(); i++) {
    ...
    if (tensor.dtype() != first.dtype()) {
      return absl::InvalidArgumentError(MismatchedElementsMsg(
...
absl::StatusOr<tf::GraphDef> DatasetFromTensorStructuresGraph(
    TensorStructures tensor_structures) {
  size_t num_structures = tensor_structures.size();
  if (num_structures == 0) {
  ...
  for (size_t element_index = 0; element_index < elements_per_structure;
       element_index++) {
    std::pair<tf::DataType, tf::TensorShape> dtype_and_shape = TFF_TRY(
        GetDtypeAndShapeForStructureElement(tensor_structures, element_index));
    tf::DataType dtype = dtype_and_shape.first;
    tf::TensorShape shape = dtype_and_shape.second;
  ...
  tf::NodeBuilder ds_to_graph_builder(kOutputTensorName, "DatasetToGraphV2");
...
absl::StatusOr<tf::Tensor> DatasetFromTensorStructures(
  ...
  SessionProvider session_provider(std::move(graph_def), std::nullopt);
  auto session = TFF_TRY(session_provider.BorrowSession());
  std::vector<tf::Tensor> outputs;
  tensorflow::Status status =
      session->Run(inputs, {std::string(kOutputTensorName)},
